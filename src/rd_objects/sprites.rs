...
use opengl_graphics::{TextureSettings, GlGraphics, Texture};
use graphics::{
	// Shapes.
...
impl<'a, T:Graphics> Sprite<'a, T> {
	pub fn new(gbe:&'a mut T) -> Self {
		Sprite::<'a, T> {x: 0., y: 0., w: 0., h: 0.,
			gfxBackend: gbe, drawState: DrawState::default(),
...
impl<G: Graphics<Texture = Texture>> SpriteDescriptor for Sprite<'_, G> {
	fn draw(&mut self, tf:Matrix2d) -> () {
		match &self.portrait {
...
/*impl<T:Graphics> Positioned for Sprite<'_, T> {
	//
}*/