...
use super::position::{Positioned, Girthy};
...
impl Entity {
	pub fn create(
		name:String, x:Scalar, y:Scalar,
		w:Scalar, h:Scalar
	) -> Self {
		Self {
			velocityX: 0., velocityY: 0.,
			isGhost: true, maxHp: 0,
			name:Option::Some(name), hp: 0,
			x, y, w, h
		}
	}
...
impl EntityDescriptor for Entity {
	fn hasCollision(&self)             -> bool { self.isGhost }
	fn setCollision(&mut self, f:bool) -> ()   { self.isGhost = f; }
	
	fn damage(&mut self, amt:i32)      -> i32  {
		self.hp -= amt;
		self.hp
	}
	fn getHP(&self)                    -> i32  { self.hp }
	fn setHP(&mut self, amt:i32)       -> ()   { self.hp = amt; }
}
